function  [Pxx,Pyy,Pxy,Txy,Cxy,Exx,Eyy,ETxy,pX,pY,pXY,f,DOFxys,zsl,Ep_gf,EC_gf,r]= mt_cspek_phs(x,y,Fs,e,v,unc);
%
...
% Calculation of the phase estimate error at a given frequency (gf): Ep_gf
gf = 1.9322736; % M2 frequency in cpd
[errorf,idx] = min(abs(f-gf));
jackstatp = jackknife(@mean,eigcr(idx,:));
pXYgf  = angle(jackstatp);
Ep_gf = std(pXYgf);

% Calculation of the coherency error at a given frequency (gf): EC_gf
jackstatSxx = jackknife(@mean,eigsp_x(idx,:));
jackstatSyy = jackknife(@mean,eigsp_y(idx,:));
jackstatSxy = jackknife(@mean,eigcr(idx,:));
