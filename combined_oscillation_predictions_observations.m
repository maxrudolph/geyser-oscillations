...
for conduit_diameter = [1 2]
    ...
    switch geometry
        case 0
            % note - this is the tiny 'practice geyser'
        ...
        case 1
            % this is the plastic tank used through 2024.
    ...
    par.rho = 960; % water density in kg*m^-3
    ...
    p2=errorbar(labfrequencies(1,:), exp_frequencies,err,err,propagated_errors(1,:),propagated_errors(1,:),'LineStyle','none','Marker','none'); hold on
    p2(1).MarkerSize = 15;
    ...
    if conduit_diameter == 1
        plot([0.3 1.05],[0.3 1.05],'k--');
    end
    % filled = conduit_diamer == 1;
    htmp = scatter(labfrequencies(1,:),exp_frequencies,[],colors,symbol,'filled');
    h = [h htmp];
    set(gca,'XLim',[0.3 1.05])
...
set(gca,'XLim',[0.3,1.05],'YLim',[0.3 1.05]);
box on
h1 = scatter(0,0,[],'k','^','Filled');
h2 = scatter(0,0,[],'k','o','Filled');


legend([h1 h2],{'2.54 cm diameter','5.08 cm diameter'},'Location','northwest','FontSize',14);
xlabel('Predicted Frequency (Hz)','fontsize',14);%,'interpreter','latex');
ylabel('Observed Frequency (Hz)','fontsize',14);%'interpreter','latex');

exportgraphics(gcf,'frequencies_pred_obs_combined.pdf');
